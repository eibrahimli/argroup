...
@$services = mysqli_query($conn, "SELECT * FROM `services` where `id` = $id");
 ...
 @$servicesay = mysqli_num_rows($services);


if(!$id or $servicesay==0){
    ?>
   
    <meta http-equiv="refresh" content="0; url=<?= $siteurl ?>xidmetler"/>
            ...
            <div class="banner_inner d-flex align-items-center">
                ...
                <div class="container">
                    <div class="banner_content text-center">
                        ...
                        <div class="page_link">
                            <a href="<?= $siteurl ?>xidmetler"><?php echo $l[2]; ?></a>
                            <a href="<?php echo $pg['url']; ?>"><?php echo $pg['title_'.$lang.'']; ?></a>
                            <a href="xidmet/<?= seo($service['name_'.$lang.''])."/".$service['id']; ?>"><?php echo $service['name_'.$lang.'']; ?></a>
                        </div>
                    </div>
                </div>
            </div>
...
<?php  } ?>
      ...
      <?php if($service['ytb']){ ?> <?php echo $service['ytb']; ?><?php }
       $video2tap = mysqli_query($conn, "SELECT * FROM `service_video` WHERE `service` = $id");
       while($video2 = mysqli_fetch_array($video2tap)){
           echo "<br/>".$video2['video'];
       }
      ?>
</div>
</div></div></section>
